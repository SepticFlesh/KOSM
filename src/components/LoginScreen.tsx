...
export function LoginScreen({ onLogin, loading, error }: LoginScreenProps) {
  return (
    <div style={{
      ...
      {loading ? (
        <div style={{ color: '#f84', textAlign: 'center' }}>
          <div style={{ fontSize: 16, marginBottom: 8 }}>Connecting...</div>
          <div style={{ fontSize: 11, opacity: 0.6 }}>Complete authorization in the popup window</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button onClick={() => onLogin('google')} style={{
            background: '#fff', color: '#333', border: '1px solid #ddd',
            borderRadius: 6, padding: '10px 28px', fontSize: 14, cursor: 'pointer',
          ...
          <button onClick={() => onLogin('github')} style={{
